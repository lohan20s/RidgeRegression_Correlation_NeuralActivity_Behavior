function [Comb,DataS]= combineData_spontFacePupil(OverallDir,outputFolder,MiceAnalyze,params,Condition)
%% This function oragnizes imaging data fron spontaneous sessions into trials around state transition or airpuff events,
%combines data across sessions for each mouse and pools data acroos mice
%Sweyta Lohani,2020
%% inputs
...
for z=1:length(MiceAnalyze)
    ...
    for folder =1: length(DirFolders)
        ...
        %extract face and pupil state timestamps
        indivFigureFolder=fullfile(outputFolder,MiceAnalyze{z},DirFolders(folder).name);
        if ~exist(indivFigureFolder,'dir'),mkdir(indivFigureFolder), end
        [tstamps]=extractFacePupilTStamps(params,channels_data,proc,timestamps,timing,indivFigureFolder);
        
        %extract the current session's data for specified color wavelengths
        for colorLen=1:numel(colors)
            currColor=colors{colorLen};
            %extract face trials
            if ~isempty(tstamps.faceOn)
                [DataS.(currColor){z}]=extractSessionData(folder,DataS.(currColor){z},currColor,params,dFoF,dFoF_parcells,proc,Idx,timing, channels_data,tstamps.faceOn,timestamps.timaging,'face',params.preEventWin,params.postEventWin, params.baselineWin);
            end
            %extract pupil trials
            if isfield(tstamps,'pupilOn')
                if ~isempty(tstamps.pupilOn)
                    [DataS.(currColor){z}]=extractSessionData(folder,DataS.(currColor){z},currColor,params,dFoF,dFoF_parcells,proc,Idx,timing, channels_data,tstamps.pupilOn,timestamps.timaging,'pupil',params.preEventWin,params.postEventWin, params.baselineWin);
                end
            end
        end
    ...
    for colorLen=1:numel(colors)
        currColor=colors{colorLen};
        if isfield(DataS.(currColor){z},'face')
            [Comb.(currColor)]= combineSessionData(z,Comb.(currColor),DataS.(currColor){z},'face');%face on trials
        end
        if isfield(DataS.(currColor){z},'pupil')
            [Comb.(currColor)]= combineSessionData(z,Comb.(currColor),DataS.(currColor){z},'pupil');%pupil on trials
        end
    end
end
